...
function getNextFriday(date) {
  ...
  const curDay = new Date(date).getDay().toString();
  const daysTillFriday = obj[curDay];
  const msInWeek = daysTillFriday * 24 * 60 * 60 * 1000;
  const day = Date.parse(date) + msInWeek;
  return new Date(day);
}
...
function formatDate(date) {
  ...
  const ms = Date.parse(dateForm);
  const year = dateForm.getFullYear();
  const month = dateForm.getMonth() + 1;
  const day = dateForm.getDate();
  const hours = Math.round(ms / 86400000) % 12;

  const minutes = dateForm.getMinutes();
  const part = hours >= 12 ? 'AM' : 'PM';
  return `${month}/${day}/${year}, ${hours}:${minutes.toString().padStart(2, 0)}:00 ${part}`;
}
...
function isLeapYear(date) {
  ...
  const isLeap = new Date(year, 2, 0);
  return isLeap.getDate() === 29;
}
